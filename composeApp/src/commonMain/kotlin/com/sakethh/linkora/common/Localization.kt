...
object Localization {
    ...
    enum class Key(val defaultValue: String) {
        ...
        Layout(defaultValue = "Layout"), Language(defaultValue = "Language"), Data(defaultValue = "Data"), Privacy(
            defaultValue = "Privacy"
        ),
        About(defaultValue = "About"), Acknowledgments(defaultValue = "Acknowledgments"), UseDarkMode(
            defaultValue = "Use Dark Theme"
        ),
        AppLanguage(defaultValue = "App Language"), DisplayingRemoteStrings(defaultValue = "Displaying Remote Strings"), ResetAppLanguage(
            defaultValue = "Reset App Language"
        ),
        AvailableLanguages(defaultValue = "Available Languages"), LoadServerStrings(defaultValue = "Load Server Strings"), LoadCompiledStrings(
            defaultValue = "Load Compiled Strings"
        ),
        UpdateLanguageStrings(defaultValue = "Update Language Strings"), DownloadLanguageStrings(
            defaultValue = "Download Language Strings"
        ),
        RemoveLanguageStrings(defaultValue = "Remove Language Strings"), DisplayingCompiledStrings(
            defaultValue = "Displaying Compiled Strings"
        ),
        Home(defaultValue = "Home"), Search(defaultValue = "Search"), History(defaultValue = "History"), HistoryLinks(
        ...
        AddANewLinkInImportantLinks(defaultValue = "Add a new link in Important Links"), AddANewLinkInSavedLinks(
            defaultValue = "Add a new link in Saved Links"
        ),
        AddANewLinkIn(defaultValue = "Add a new link in ${LinkoraPlaceHolder.First.value}"), LinkAddress(
            defaultValue = "Link Address"
        ),
        TitleForTheLink(defaultValue = "Title for the link"), NoteForSavingTheLink(defaultValue = "Note for saving the link"), AutoDetectTitleIsEnabled(
            defaultValue = "Auto Detect Title is currently active."
        ),
        DataRetrievalDisabled(defaultValue = "Data retrieval is blocked as the 'Force Save Links without retrieval' feature is currently active."), ForceAutoDetectTitle(
            defaultValue = "Force Auto-Detect Title"
        ),
        RetryingWithSecondaryUserAgent(defaultValue = "Retrying metadata retrieval using a secondary user agent."), ForceSaveWithoutRetrievingMetadata(
            defaultValue = "Force Save Without Retrieving Metadata"
        ),
        AddIn(defaultValue = "Add in"), InitialRequestFailed(defaultValue = "he initial request failed."), ImportantLinks(
            defaultValue = "Important Links"
        ),
        Save(defaultValue = "Save"), SaveInThisFolder(defaultValue = "Save in this folder"), Cancel(
            defaultValue = "Cancel"
        ),
        FolderName(defaultValue = "Folder Name"), NoteForCreatingTheFolder(defaultValue = "Note For Creating The Folder"), AllLinks(
            defaultValue = "All Links"
        ),
        Links(defaultValue = "Links"), Archive(defaultValue = "Archive"), ArchiveLinks(defaultValue = "Archive Links"), Folders(
            defaultValue = "Folders"
        ),
        SuccessfullySavedConnectionDetails(defaultValue = "Successfully saved connection details."), DeletedTheServerConnectionSuccessfully(
            defaultValue = "Deleted the server connection successfully."
        ),
        UseInAppBrowser(defaultValue = "Use In-App Browser"), UseInAppBrowserDesc(defaultValue = "Enable this to open links within the app; otherwise, your default browser will open when clicking on links."), EnableHomeScreen(
            defaultValue = "Enable Home Screen"
        ),
        EnableHomeScreenDesc(defaultValue = "If this is enabled, Home Screen option will be shown in Bottom Navigation Bar; if this setting is not enabled, Home screen option will NOT be shown."), AutoDetectTitle(
            defaultValue = "Auto-Detect Title"
        ),
        AutoDetectTitleDesc(defaultValue = "Note: This may not detect every website."), ForceSaveWithoutRetrievingMetadataDesc(
        ...
        ShowAssociatedImageInLinkMenu(defaultValue = "Show associated image in link menu"), ShowAssociatedImageInLinkMenuDesc(
            defaultValue = "Enables the display of an associated image within the link menu."
        ),
        AutoCheckForUpdates(defaultValue = "Enables the display of an associated image within the link menu."), AutoCheckForUpdatesDesc(
            defaultValue = "Enable to auto-check for updates on app launch. Disable for manual checks."
        ),
        ShowDescriptionForSettings(defaultValue = "Show description for Settings"), ShowDescriptionForSettingsDesc(
            defaultValue = "Enable to show detailed descriptions for settings. Disable to show only titles."
        ),
        ManageConnectedServer(defaultValue = "Manage Connected Server"), ManageConnectedServerDesc(
            defaultValue = "Your data is synced with the Linkora server. Tap to manage or disconnect."
        ),
        CurrentlyConnectedTo(defaultValue = "Currently Connected To"), SyncType(defaultValue = "Sync Type"), EditServerConfiguration(
            defaultValue = "Edit server configuration"
        ),
        DeleteTheServerConnection(defaultValue = "Delete the connection"), Configuration(
            defaultValue = "Configuration"
        ),
        ServerURL(defaultValue = "Server URL"), ServerSetupInstruction(defaultValue = "Ensure the server is running. If hosted locally, the server URL should include the correct port number. No port is needed if the server is not hosted locally."), SecurityToken(
            defaultValue = "Security Token"
        ),
        ServerIsReachable(defaultValue = "Server Exists and Is Reachable!"), TestServerAvailability(
            defaultValue = "Test Server Availability"
        ),
        UseThisConnection(defaultValue = "Use This Connection"), ClientToServer(defaultValue = "Client To Server"), ClientToServerDesc(
            defaultValue = "Client changes are sent to the server, but client is not updated with server changes."
        ),
        ServerToClient(defaultValue = "Server To Client"), ServerToClientDesc(defaultValue = "Server changes are sent to the client, but server is not updated with client changes."), TwoWaySync(
        ...
        ExportDataAsJson(defaultValue = "Export Data as JSON"), ExportDataAsJsonDesc(defaultValue = "Export All Data to a JSON File"), ExportDataAsHtml(
            defaultValue = "Export Data as HTML"
        ),
        ExportDataAsHtmlDesc(defaultValue = "Export All Your Data (Excluding Panels) as HTML File"), Sync(
            defaultValue = "Sync"
        ),
        ConnectToALinkoraServer(defaultValue = "Connect to a Linkora Server"), ConnectToALinkoraServerDesc(
        ...
        RefreshingLinksAndroidDesc(defaultValue = "Closing Linkora won\'t interrupt link refreshing, but newly added links might not be processed."), RefreshingLinksDesktopDesc(
            defaultValue = "Closing Linkora will cancel link refreshing."
        ),
        InitialScreenOnLaunch(defaultValue = "Initial Screen on Launch"), InitialScreenOnLaunchDesc(
        ...
        Confirm(defaultValue = "Confirm"), SelectTheInitialScreen(defaultValue = "Select the initial screen on launch"), ShowBorderAroundLinks(
        ...
        ChooseTheLayoutYouLikeBest(defaultValue = "Choose the layout you like best"), FeedPreview(
            defaultValue = "Feed Preview"
        ),
        RegularListView(defaultValue = "Regular List View"), TitleOnlyListView(defaultValue = "Title Only List View"), GridView(
            defaultValue = "Grid View"
        ),
        StaggeredView(defaultValue = "Staggered View"), FollowSystemTheme(defaultValue = "Follow System Theme"), UseDynamicTheming(
            defaultValue = "Use dynamic theming"
        ),
        UseDynamicThemingDesc(defaultValue = "Change colour themes within the app based on your wallpaper."), UseAmoledTheme(
            defaultValue = "Use Amoled Theme"
        ),
        RetrieveLanguageInfoFromServer(defaultValue = "Retrieve Language Info from Server"), SelectACollection(
            defaultValue = "Select a Collection"
        ),
        SelectAPanel(defaultValue = "Select a Panel"), FolderHasBeenCreatedSuccessful(defaultValue = "The folder ${LinkoraPlaceHolder.First.value} has been successfully created."), RemoteExecutionFailed(
            defaultValue = "Remote execution failed :"
        ),
        SavedAvailableLanguagesInfoLocally(defaultValue = "Saved Available Languages Info locally."), DeletedTheStringsPack(
            defaultValue = "Deleted the ${LinkoraPlaceHolder.First.value} strings pack."
        ),
        DownloadedLanguageStrings(defaultValue = "Downloaded Language Strings for the ${LinkoraPlaceHolder.First.value}."), Linkora(
            defaultValue = "Linkora"
        ),
        LinkoraIsConnectedToAServer(defaultValue = "Linkora is connected to the server; syncing is based on ${LinkoraPlaceHolder.First.value}."), CopiedLinkToClipboard(
            defaultValue = "Copied Link to Clipboard"
        ),
        FetchingAvailableLanguages(defaultValue = "Fetching Available Languages"), DownloadingStrings(
            defaultValue = "Downloading Strings for ${LinkoraPlaceHolder.First.value}"
        ),
        CopiedTitleToTheClipboard(defaultValue = "Copied Title to The Clipboard"), CopiedNoteToTheClipboard(
            defaultValue = "Copied Note to The Clipboard"
        ),
        ViewNote(defaultValue = "View Note"), Rename(defaultValue = "Rename"), RefreshImageAndTitle(
            defaultValue = "Refresh Image And Title"
        ),
        Refresh(defaultValue = "Refresh"), UnArchive(defaultValue = "Unarchive"), UnArchived(
            defaultValue = "Unarchived and saved to the default \"Saved Links\" collection."
        ),
        DeleteTheNote(defaultValue = "Delete The Note"), MoveToRootFolders(defaultValue = "Move To Root Folders"), DeleteTheLink(
            defaultValue = "Delete the Link"
        ),
        DeletedTheLink(defaultValue = "Deleted the Link"), CopyFolder(defaultValue = "Copy Folder"), MoveToOtherFolder(
            defaultValue = "Move To Other Folder"
        ),
        CopyLink(defaultValue = "Copy Link"), MoveLink(defaultValue = "Move Link"), DeleteTheFolder(
            defaultValue = "Delete The Folder"
        ),
        DeletedTheFolder(defaultValue = "Folder ${LinkoraPlaceHolder.First.value}, all internal folders, and associated links have been successfully deleted."), DeletedTheNoteOfAFolder(
            defaultValue = "Successfully deleted the note of the ${LinkoraPlaceHolder.First.value}."
        ),
        DeletedTheNoteOfALink(defaultValue = "Successfully deleted the note."), FolderDeletionLabel(
            defaultValue = "Deleting this folder will also remove all its subfolders"
        ),
        AreYouSureDeleteSelectedLinks(defaultValue = "Are you sure you want to delete all selected links?"), AreYouSureDeleteLink(
            defaultValue = "Are you sure you want to delete the link?"
        ),
        AreYouSureDeleteSelectedFolders(defaultValue = "Are you sure you want to delete all selected folders?"), AreYouSureDeleteFolder(
            defaultValue = "Are you sure you want to delete the folder?"
        ),
        AreYouSureDeleteSelectedItems(defaultValue = "Are you sure you want to delete all selected items?"), AreYouSureDeleteEverything(
            defaultValue = "Are you sure you want to delete all folders, links, panels, and localized strings?"
        ),
        SavedNote(defaultValue = "Saved Note"), NoNoteAdded(defaultValue = "You haven't added a note for this."), Delete(
            defaultValue = "Delete"
        ),
        ArchivedTheFolder(defaultValue = "The folder ${LinkoraPlaceHolder.First.value} has been archived."), UnArchivedTheFolder(
            defaultValue = "The folder ${LinkoraPlaceHolder.First.value} has been unarchived."
        ),
        ArchivedTheLink(defaultValue = "Link has been archived."), ChangeBothNameAndNote(
            defaultValue = "Change Name and Note"
        ),
        ChangeNoteOnly(defaultValue = "Change Note"), RenameFolder(defaultValue = "Rename Folder ${LinkoraPlaceHolder.First.value}:"), ChangeLinkData(
            defaultValue = "Change Link data:"
        ),
        NewName(defaultValue = "New Name"), NewTitle(defaultValue = "New Title"), NewNote(
            defaultValue = "New Note"
        ),
        UpdatedTheNote(defaultValue = "Updated the note successfully."), UpdatedTheName(defaultValue = "Updated the name successfully."), UpdatedTheTitle(
            defaultValue = "Updated the title successfully."
        ),
        UpdatedTheFolderData(defaultValue = "Folder data has been updated successfully."), InvalidLink(
            defaultValue = "The link provided is invalid. Please check and try again."
        ),
        SavedTheLink(defaultValue = "Saved the link successfully."), DeletionInProgress(defaultValue = "Deletion In Progress.."), RemoveALinkFromImpLink(
            defaultValue = "Remove from Important Links"
        ),
        MarkALinkAsImpLink(defaultValue = "Mark Link as Important"), NewestToOldest(defaultValue = "Newest to Oldest"), OldestToNewest(
            defaultValue = "Oldest to Newest"
        ),
        AToZSequence(defaultValue = "A to Z Sequence"), ZToASequence(defaultValue = "Z to A Sequence"), SortFoldersBy(
            defaultValue = "Sort folders by"
        ),
        SortHistoryLinksBy(defaultValue = "Sort History Links by"), SortBy(defaultValue = "Sort by"), SortSavedLinksBy(
            defaultValue = "Sort Saved Links by"
        ),
        SortImportantLinksBy(defaultValue = "Sort Important Links by"), SortBasedOn(defaultValue = "Sort Based on"), SearchTitlesToFindLinksAndFolders(
            defaultValue = "Search titles or notes to find links and folders"
        ),
        RegularFolder(defaultValue = "Regular Folder"), ArchiveFolder(defaultValue = "Archive Folder"), GoodMorning(
            defaultValue = "Good Morning"
        ),
        GoodAfternoon(defaultValue = "Good Afternoon"), GoodEvening(defaultValue = "Good Evening"), HeyHi(
            defaultValue = "Hey, hi👋"
        ),
        Default(defaultValue = "Default"), SelectedPanel(defaultValue = "Selected Panel"), AddANewPanel(
            defaultValue = "Add A New Panel"
        ),
        Panels(defaultValue = "Panels"), FoldersInThisPanel(defaultValue = "Folders in This Panel"), FoldersThatCanBeAddedToThisPanel(
            defaultValue = "Folders that can be added to this panel"
        ),
        PanelName(defaultValue = "Panel Name"), PermanentlyDeleteThePanel(defaultValue = "Permanently Delete Panel"), OnceDeletedThisPanelCannotBeRestored(
            defaultValue = "Once deleted, this Panel cannot be restored."
        ),
        AreYouSureWantToDeleteThePanel(defaultValue = "Are you sure want to delete the panel named ${LinkoraPlaceHolder.First.value}?"), NewNameForPanel(
            defaultValue = "New Name for Panel"
        ),
        ChangePanelName(defaultValue = "Change Panel Name"), EditPanelName(defaultValue = "Edit ${LinkoraPlaceHolder.First.value} Panel Name"), RedirectToLatestReleasePage(
        ...
        GithubDesc(defaultValue = "The Linkora app, sync server, and localization server are public and open-source—feel free to explore the code."), Github(
            defaultValue = "Github"
        ),
        Discord(defaultValue = "Discord"), Twitter(defaultValue = "Twitter"), Development(
            defaultValue = "Development"
        ),
        Socials(defaultValue = "Socials"), YouAreUsingLatestVersionOfLinkora(defaultValue = "You are using latest version of Linkora."), CheckForLatestVersion(
        ...
        SerializingCollectedDataForExport(defaultValue = "Serializing the collected data for export..."), ImportExportScreenTopAppBarDesc(
            defaultValue = "Stay on this page! DO NOT PANIC IF IT LOOKS STUCK."
        ),
        ExportedSuccessfully(defaultValue = "Exported Successfully"), StoragePermissionIsRequired(
            defaultValue = "Storage permission is required to store or export the file. Please grant the permission to proceed."
        ),
        NotificationPermissionIsRequired(defaultValue = "Enable notification permission to view the progress of link refreshes."), PermissionGranted(
            defaultValue = "Permission granted. Please retry the action to continue with your import/export."
        ),
        LinkRefreshedSuccessfully(defaultValue = "The link data has been successfully refreshed."), Share(
            defaultValue = "Share"
        ),
        ForceOpenInABrowser(defaultValue = "Open In A Browser"), WorkManagerDesc(defaultValue = "Work Manager is scheduling the links refreshing task. It will continue shortly.\nYou can close the app; this task will continue in the background."), NoOfLinksRefreshed(
            defaultValue = "${LinkoraPlaceHolder.First.value} of ${LinkoraPlaceHolder.Second.value} links refreshed."
        ),
        TopDecoratorSetting(defaultValue = "Use Linkora's Top Decorator"), TopDecoratorSettingDesc(
            defaultValue = "When disabled, the default decorator will be used. Changes will apply on the next launch."
        ),
        ThisFolderHasNoSubfolders(defaultValue = "This folder does not contain any subfolders."), PanelCreatedSuccessfully(
            defaultValue = "Panel created successfully with the name: ${LinkoraPlaceHolder.First.value}"
        ),
        DeletedPanelSuccessfully(defaultValue = "Panel deleted successfully."), UpdatedThePanelNameSuccessfully(
            defaultValue = "Panel name updated to: ${LinkoraPlaceHolder.First.value}."
        ),
        ImportingDataFromTheSever(defaultValue = "Importing Data from the Remote Server"), ImportingDataFromTheSeverDesc(
