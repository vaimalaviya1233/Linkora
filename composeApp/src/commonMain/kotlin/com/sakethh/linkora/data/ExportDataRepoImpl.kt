...
class ExportDataRepoImpl(
    ...
    override suspend fun exportDataAsHTMl(): Flow<Result<RawExportString>> {
        return flow {
            supervisorScope {
                ...
                emit(Result.Loading(message = "Starting export data as HTML"))

                val allLinks = localLinksRepo.getAllLinks()
                emit(Result.Loading(message = "Fetched all links, total: ${allLinks.size}"))

                var savedLinksSection = dtH3(LinkoraExports.SAVED_LINKS__LINKORA_EXPORT.name)
                emit(Result.Loading(message = "Processing saved links"))
                ...
                val deferredSavedLinks = async {
                    allLinks.filter { it.linkType == LinkType.SAVED_LINK }.forEach { savedLink ->
                        ...
                        emit(Result.Loading(message = "Processed saved link: ${savedLink.title}"))
                    }
                }

                var impLinksSection = dtH3(LinkoraExports.IMPORTANT_LINKS__LINKORA_EXPORT.name)
                emit(Result.Loading(message = "Processing important links"))
                ...
                val deferredImpLinks = async {
                    allLinks.filter { it.linkType == LinkType.IMPORTANT_LINK }.forEach { impLink ->
                        ...
                        emit(Result.Loading(message = "Processed important link: ${impLink.title}"))
                    }
                }

                emit(Result.Loading(message = "Processing regular folders and respective links"))
                ...
                emit(Result.Loading(message = "Processing archived folders and respective links"))
                ...
                emit(Result.Loading(message = "Processing history links"))
                ...
                val deferredHistoryLinks = async {
                    allLinks.filter { it.linkType == LinkType.HISTORY_LINK }
                        .forEach { historyLink ->
                            ...
                            emit(Result.Loading(message = "Processed history link: ${historyLink.title}"))
                        }
                }

                var archivedLinksSection = dtH3(LinkoraExports.ARCHIVED_LINKS__LINKORA_EXPORT.name)
                emit(Result.Loading(message = "Processing archived links"))
                ...
                val deferredArchivedLinks = async {
                    allLinks.filter { it.linkType == LinkType.ARCHIVE_LINK }
                        .forEach { archivedLink ->
                            ...
                            emit(Result.Loading(message = "Processed archived link: ${archivedLink.title}"))
                        }
                }
                ...
                emit(Result.Loading(message = "Completed export process, finalizing HTML"))
                linkoraLog(dlP(htmlFileRawText))

                emit(Result.Success(dlP(htmlFileRawText)))
            }
        }
    }
    ...
    private suspend fun <T> FlowCollector<Result<T>>.foldersSectionInHtml(
        ...
        val foldersList = if (parentFolderId == null) {
            emit(Result.Loading(message = "Fetching all top-level folders"))
            localFoldersRepo.getAllFoldersAsList().filter {
                it.isArchived == forArchiveFolders
            }
        } else {
            emit(Result.Loading(message = "Fetching child folders for parent ID: $parentFolderId"))
            localFoldersRepo.getChildFoldersOfThisParentIDAsList(parentFolderId)
        }

        emit(Result.Loading(message = "Found ${foldersList.size} folders to process"))

        foldersList.forEach { childFolder ->
            emit(Result.Loading(message = "Processing folder: ${childFolder.name} (ID: ${childFolder.localId})"))

            val currentFolderDTH3 = dtH3(childFolder.name)
            var folderLinksDTA = ""

            val linksList = localLinksRepo.getLinksOfThisFolderAsList(childFolder.localId)
            emit(Result.Loading(message = "Found ${linksList.size} links in folder: ${childFolder.name}"))

            linksList.forEach { filteredLink ->
                emit(Result.Loading(message = "Processing link: ${filteredLink.title} (URL: ${filteredLink.url}) in folder: ${childFolder.name}"))
                folderLinksDTA += dtA(linkTitle = filteredLink.title, link = filteredLink.url)
            }

            emit(Result.Loading(message = "Completed links for folder: ${childFolder.name}"))
            ...
            if (childFolder.parentFolderId == null) {
                if (forArchiveFolders) {
                    emit(Result.Loading(message = "Top-level archived folder ${childFolder.name} processed"))
                } else {
                    emit(Result.Loading(message = "Top-level non-archived folder ${childFolder.name} processed"))
                }
            }

            emit(Result.Loading(message = "Finished processing folder: ${childFolder.name} (ID: ${childFolder.localId})"))
        }
